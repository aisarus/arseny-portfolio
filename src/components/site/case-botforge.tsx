  ...
  return (
    <section id="botforge" className="on-ink signal-section botforge-v3 scroll-mt-16 bg-background py-20 text-foreground sm:py-28">
      ...
      <Shell className="signal-content">
        ...
        <div className="botforge-broadcast mt-14" style={{ "--lens-x": `${lens.x}%`, "--lens-y": `${lens.y}%` } as CSSProperties} onPointerMove={(event) => { if (isStatic) return; const bounds = event.currentTarget.getBoundingClientRect(); setLens({ x: ((event.clientX - bounds.left) / bounds.width) * 100, y: ((event.clientY - bounds.top) / bounds.height) * 100 }); }}>
